...
from server.app.domain.scoring.weight_utils import (
    validate_weights,
...
def test_apply_default_weights_partial():
    ...
    assert result["lighting"] == 1.0  # default
...
def test_apply_default_weights_non_numeric_fallback():
    ...
    assert result["distance"] == 1.0  # fallback to default

# Scoring functions
def make_row(**kwargs):
    """Helper to create a fake GeoDataFrame row."""
    return kwargs
...
def test_calculate_weights_full_pipeline(tmp_path, monkeypatch):
    # Prevent CSV writing during tests
    monkeypatch.setattr(
        "server.app.domain.scoring.weight_utils.gpd.GeoDataFrame.to_csv",
